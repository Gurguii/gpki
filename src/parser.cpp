...
int Parse(int argc, const char **&_args)
{
  ...
  if(!std::filesystem::exists(Globals::profiles_file)){
    ...
    if(!std::filesystem::exists(Globals::profiles_file)){
      std::cerr << "[error] couldn't create missing .profiles file -> " << Globals::profiles_file;
      return -1;
    }else{std::cout << Globals::profiles_file << " file created\n";}
  }
  ...
  if(args.size() == 1){
    int rcode = -1;
    // Check if options have an action that does not require profile name (init-pki)
