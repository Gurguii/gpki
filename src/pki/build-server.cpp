...
int build_server(std::string_view common_name) {
  ...
  std::string command =
      "openssl req -config " + std::string(info.openssl_config) + "-new -out " +
      std::string(info.reqs) + SLASH + common_name.data() + " -keyout" +
      std::string(info.keys) + SLASH + common_name.data();
