...
int GetCertCreationInfo(std::string_view profile_name, CertCreationInfo &buff,
  ...
  std::ifstream(std::string(info.serial) + SLASH + "serial").read(buff.serial, sizeof(buff.serial));

  // remove terminating \n from serial to be able to properly format the string
  buff.serial[strlen(buff.serial) - 1] = 0x00;
  snprintf(buff.csr, sizeof(buff.csr), "%s%s%s-csr.pem", info.reqs,
           SLASH, buff.serial);
  snprintf(buff.crt, sizeof(buff.crt), "%s%s%s-crt.pem", info.certs,
           SLASH, buff.serial);
  snprintf(buff.key, sizeof(buff.key), "%s%s%s-key.pem", info.keys,
           SLASH, buff.serial);
  // printf("buff.csr -> %s\nbuff.crt -> %s\nbuff.key -> %s\n", buff.csr,
  // buff.crt,buff.key);
  if (type == x509_server) {
    ...
    snprintf(buff.csr_command, sizeof(buff.csr_command),
             ...
             info.openssl_config, Globals::keysize, Globals::keyalgorithm.c_str(),Globals::outformat.c_str(), buff.csr, buff.key);
    // crt signing command
    snprintf(buff.crt_command, sizeof(buff.crt_command),
             "openssl ca -config %s -in %s -out %s -extfile %s %s",
             info.openssl_config, buff.csr, buff.crt,
             (std::string(info.x509) + SLASH + "server").c_str(), Globals::prompt ? "\x00" : "--notext -batch");
    ...
    snprintf(buff.crt_command, sizeof(buff.csr_command),
             "openssl ca -config %s -in %s -out %s -extfile %s %s",
             info.openssl_config, buff.csr, buff.crt,
             (std::string(info.x509) + SLASH + "client").c_str(), Globals::prompt ? "\x00" : "--notext -batch");
  }
  return 0;
};
int GetCertCreationInfo(std::string_view profile_name, CertCreationInfo &buff,
